...
def predict():
    ...
    if (perc >= 0 and perc < 30):
        res['data'] = f'''Вероятность АКТГ-эктопированного синдрома: {perc}%. 
        Полученный результат указывает на низкую вероятность АКТГ-эктопированного синдрома.'''
    if (perc >= 30 and perc < 50):
        res['data'] = f'''Вероятность АКТГ-эктопированного синдрома: {perc}%.
        Полученный результат не позволяет исключить наличие у пациента АКТГ-эктопированного синдрома.'''
    if (perc >= 50 and perc < 70):
        res['data'] = f'''Вероятность АКТГ-эктопированного синдрома: {perc}%. 
        Полученный результат не позволяет исключить наличие у пациента болезни Иценко-Кушинга.'''
    if (perc >= 70 and perc <= 100):
        res['data'] = f'''Вероятность АКТГ-эктопированного синдрома: {perc}%. 
        Полученный результат указывает на высокую вероятность АКТГ-эктопированного синдрома.'''
    return res
